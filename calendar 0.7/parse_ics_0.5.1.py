...
def add_event(source_type, event_name, date, color, symbol, append):
    date = date.split('-')
...
def parse_csv_file(source_type, event_name, filename, color, symbol, append):
    ...
    for line in csv_file:
        ...
        line=line.rstrip('\n').split(',')
        if line[1] == '': continue
        event_summary = line[0]
    	date = line[1]
        date = date.split('-')
        event_date = datetime.date(int(date[0]), int(date[1]), int(date[2]))

        #fetch leap day
	if event_date.month == 2 and event_date.day == 29 and not calendar.isleap(this_year): continue

        # calculate anniversaries
        if event_name == "anniversary":
            age = this_year - event_date.year
            print "Anniversary: " + event_summary + " / Age: " + str(age)
            event_summary += " " + str(age) + u" år"
            event_date = event_date.replace(year=this_year)

        #create a new field
        new_event = my_class.event(event_name, event_summary, color, symbol)
        my_container.get_day_by_date(event_date).add_event(new_event, append=='true')
    return 0


def parse_ics_file(source_type, event_name, filename, color, symbol, append):
    print "Parsing file for " + event_name + ":", filename
    ...
    for event in ical.vevent_list:
        print event_name, event.summary.value
...
for line in conf_file:
    ...
    elif line[0] == "csv": parse_csv_file(*line)
